...
import type { RyuCatalogModels } from "@ryu/app-host/app-bridge";
...
export async function loadRegistry(): Promise<CanvasRegistry> {
	const b = bridge();
	const [runtimeCatalog, agents, engineModels, ttsEngines] = await Promise.all([
		b.catalog?.snapshot().catch(() => null) ?? Promise.resolve(null),
	...
	return {
		agents:
			runtimeCatalog?.agents.map((agent) => ({
				...
				recommended: agent.recommended ?? false,
			})) ?? agents,
		engineModels,
		discoverModels: b.catalog?.models
			? (providerId: string) =>
					b.catalog?.models({ providerId }) as Promise<RyuCatalogModels>
			: undefined,
		runtimeCatalog,
		ttsEngines,
	};
...
// per-app CSP allowlist (csp.connectDomains/resourceDomains in manifest.json) permits
...
export function searchImages(
	...
	return bridge().assets.searchImages({ provider, query });
}
