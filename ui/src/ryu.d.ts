...
export interface RyuAssets {
	...
	/** Search host-proxied Openverse or Unsplash images. Media is returned as data URLs. */
	searchImages(input: {
...
declare module "@ryu/app-host/realtime" {
	interface RyuCompanionWindowApi {
		agent: RyuBridge["agent"];
		assets: RyuBridge["assets"];
		catalog?: RyuBridge["catalog"];
		context: RyuBridge["context"];
		listAgents: RyuBridge["listAgents"];
		media: RyuBridge["media"];
		model: RyuBridge["model"];
		registry: RyuBridge["registry"];
